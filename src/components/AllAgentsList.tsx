...
// Live "All Agents" list — same markup as the original static page, driven by
// GET /agents with the state/keyword filters (from RealtorSearch → URL) and
// working pagination.
export default function AllAgentsList() {
  ...
  // Compact page window: first, current-1..current+1, last (deduped, in order).
      ...
      {isLoading ? (
        ...
        <div className="rounded-[20px] border border-[#f6f6f6] bg-[#fafafa] py-16 text-center">
          <p className="text-[14px] text-[#807e7e]">
            {isError ? "Couldn’t load agents right now." : "No agents match your search."}
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3" style={{ gap: "40px 24px" }}>
          ...
          {pageNumbers.map((p, i) => {
            ...
            return (
              <span key={p} className="flex items-center" style={{ gap: "16px" }}>
                {gap && (
                  <span style={{ fontSize: "14px", color: "#121212" }}>…</span>
                )}
                <button
                  type="button"
                  onClick={() => goToPage(p)}
